...
// Update provides a mock function with given fields: file, token, updateData
func (_m *UserService) Update(file multipart.FileHeader, token interface{}, updateData user.Core) (user.Core, error) {
	ret := _m.Called(file, token, updateData)

	var r0 user.Core
	if rf, ok := ret.Get(0).(func(multipart.FileHeader, interface{}, user.Core) user.Core); ok {
		r0 = rf(file, token, updateData)
	} else {
		r0 = ret.Get(0).(user.Core)
	}
