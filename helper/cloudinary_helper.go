...
type Product struct {
	Product multipart.File `json:"product,omitempty" validate:"required"`
}
...
type mediaUpload interface {
	...
	ProductUpload(product Product) (string, error)
}
...
func (*media) ProductUpload(product Product) (string, error) {
	//validate
	...
	//upload
	uploadUrl, err := ImageUploadHelper(product.Product)
	if err != nil {
		return "", err
	}
	return uploadUrl, nil
